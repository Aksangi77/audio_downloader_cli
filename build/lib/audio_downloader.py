...
This tool downloads audio from a YouTube playlist with embedded metadata and cover art.
If not provided as arguments, it will prompt for the playlist URL and output folder.
...
def main():
    parser = argparse.ArgumentParser(
        description="Download YouTube audio efficiently with embedded metadata and cover art."
    )
    parser.add_argument("playlist_url", nargs="?", default=None,
    ...
    if not args.output_folder:
        chosen = input("Enter the output folder (Press Enter for default '~/Music'): ").strip()
